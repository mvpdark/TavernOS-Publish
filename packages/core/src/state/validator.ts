...
import {
  StateManifestSchema, CurrentStateStateSchema, PlotThreadsStateSchema, ChapterSummariesStateSchema,
} from "../models/story-state.js";
...
function parseOrIssue<T>(
  ...
  value: unknown,
  issues: StoryStateValidationIssue[],
  code: string,
  path: string,
): T | undefined {
  try {
