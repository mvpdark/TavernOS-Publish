...
import type { OpenAIStreamChunk, AnthropicStreamEvent } from "./client-types.js";
 ...
 */
function tryExtractJsonError(rawText: string): string | null {
  ...
  try {
    ...
    if ("error_msg" in obj) return String(obj.error_msg);
...
export async function parseStreamResponse(
  ...
  let firstMalformedRaw: string | undefined;
  let totalChunks = 0;
    ...
    for (;;) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          malformedCount++;
          if (!firstMalformedRaw) firstMalformedRaw = data.slice(0, 300);
          if (malformedCount <= 3) {
  ...
  if (!content) {
    ...
    throw new PartialResponseError(
      `Stream completed but no content was received.${diag}`,
      "",
    );
  }
...
export async function parseAnthropicStreamResponse(
  ...
  if (!content) {
    ...
    throw new PartialResponseError(
      `Anthropic stream completed but no content was received.${diag}`,
      "",
    );
  }
