...
if (Math.abs(_ALLOC_SUM - 1.0) > 0.0001) {
  throw new Error(
    `InjectionPolicy budget allocation must sum to 1.0, got ${_ALLOC_SUM}`,
  );
}
...
export class InjectionPolicy {
  ...
  assemble(context: InjectionContext): AssembledPrompt {
    ...
    const pinnedSection = this.truncateSection(
      MARKER_PINNED,
      ctx.pinnedFactsBlock,
      pinnedBudget,
    );
    const innerVoiceSection = this.truncateSection(
      MARKER_INNER_VOICE,
      ctx.innerVoiceBlock,
      innerVoiceBudget,
    );
    const contextSection = this.truncateSection(
      MARKER_CONTEXT,
      ctx.contextBlock,
      contextBudget,
    );
    const timelineSection = this.truncateSection(
      MARKER_TIMELINE,
      ctx.timelineBlock,
      timelineBudget,
    );
    const paceSection = this.truncateSection(
      MARKER_PACE,
      ctx.paceRecommendation,
      paceBudget,
    );
