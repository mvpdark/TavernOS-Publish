...
const ALL_KEYWORDS = [PERSONA_KEYWORD, ...LEGACY_KEYWORDS] as const;
 ...
 * mutation) as base64.
 */
export function embedPersonaInPNG(pngBuffer: Buffer | Uint8Array, cardJson: string): Buffer {
  ...
  const cleaned = chunks.filter((chunk) => {
    ...
    return !ALL_KEYWORDS.includes(decoded.keyword.toLowerCase() as typeof ALL_KEYWORDS[number]);
  });
 ...
 */
export function extractPersonaFromPNG(pngBuffer: Buffer | Uint8Array): string {
  const src = pngBuffer instanceof Buffer ? pngBuffer : Buffer.from(pngBuffer);
  const chunks = extract(src);
...
function reassemblePNG(chunks: ReadonlyArray<{ name: string; data: Uint8Array }>): Buffer {
...
const CRC_LOOKUP_TABLE: readonly number[] = (() => {
