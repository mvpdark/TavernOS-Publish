...
import type { ImageGenRequest, ImageGenResponse, GeneratedImage } from "./types.js";

/** True when the image-generation stub is enabled via env var. */
export function isImageStubEnabled(): boolean {
 ...
 */
export function stubImageGeneration(request: ImageGenRequest): ImageGenResponse {
  const n = request.n ?? 1;
