 ...
 */
function isOptionalFunction(val: unknown): val is undefined | ((...args: never[]) => unknown) {
 ...
 */
function validateExtensionStructure(obj: Record<string, unknown>): string | null {
  ...
  if (obj.pipelineHooks !== undefined) {
    ...
    const hooks = obj.pipelineHooks as Record<string, unknown>;
  ...
  if (obj.narrativeProcessors !== undefined) {
    ...
    for (let i = 0; i < obj.narrativeProcessors.length; i++) {
      const np = obj.narrativeProcessors[i] as Record<string, unknown>;
 ...
 */
export function loadExtensionFromObject(obj: unknown): ExtensionLoadResult {
  ...
  return { success: true, extension: obj as Extension };
}
 ...
 * ```
 */
export async function loadExtensionFromModule(
  modulePath: string,
  ...
  try {
    ...
    return {
      success: false,
      error: `Failed to import module "${modulePath}": ${
 ...
 */
export function validateExtension(obj: unknown): string | null {
