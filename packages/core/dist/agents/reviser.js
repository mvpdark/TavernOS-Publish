 ...
 */
export function locateParagraph(paragraphs, location) {
    if (!location || paragraphs.length === 0)
        return -1;
 ...
 * capped. Used by the reviser so that revising N paragraphs in parallel
 ...
 */
export function createEditorAgent(ctx) {
    const runtime = createAgentRuntime(ctx);
    const name = "reviser";
    /**
     * Revise a single paragraph by sending only that paragraph + its issues.
     */
    async function reviseParagraph(paragraphText, issues, options) {
        const systemContent = "You are a revision editor. Your task is to fix the identified issues in the paragraph " +
        ...
        const issuesList = issues
            .map((issue, i) => `${i + 1}. [${issue.severity}] ${issue.dimension}: ${issue.message} ` +
            `(location: ${issue.location}, repairScope: ${issue.repairScope})`)
            .join("\n");
        const userContent = `## Paragraph to Revise\n\n${paragraphText}\n\n` +
            `## Issues to Fix\n\n${issuesList}\n\n` +
     ...
     */
    async function reviseFullText(content, issues, options) {
        const systemContent = "You are a revision editor. Your task is to fix the identified issues in the chapter content " +
            "while preserving the story's core narrative, style, and voice. " +
    ...
    async function revise(input, options) {
        ...
        const revisedResults = await mapWithConcurrency(paragraphEntries, PARAGRAPH_REVISION_CONCURRENCY, async ([paraIndex, issues]) => {
            const originalText = paragraphs[paraIndex];
            try {
                const revisedText = await reviseParagraph(originalText, issues, options);
                ...
                console.warn(`[reviser] Paragraph ${paraIndex + 1} revision failed — ` +
        ...
        if (globalIssues.length > 0) {
            content = await reviseFullText(content, globalIssues, options);
        }
        return {
