...
export function createLLMClient(config) {
    ...
    return {
        ...
        baseUrl,
        defaults,
        chat: (model, messages, options) => chatCompletion(config, model, messages, options),
    };
}
// ---------------------------------------------------------------------------
...
export async function chatCompletion(config, model, messages, options) {
    ...
    const temperature = clampTemperatureForModel(serviceName, model, options?.temperature ?? config.temperature ?? 0.7);
    ...
    return withTransientRetry(() => executeChatCompletion(config, model, messages, temperature, maxTokens, options), TRANSIENT_RETRIES);
}
// ---------------------------------------------------------------------------
// Execute — Anthropic Messages API format
// (used by Kimi Code / api.kimi.com/coding and native Anthropic /v1/messages)
// ---------------------------------------------------------------------------
async function executeAnthropicMessages(config, baseUrl, model, messages, temperature, maxTokens, useStream, options) {
    ...
    const authToken = providerRegistry.resolveAuthToken(config.service ?? "custom", config.oauthToken ?? config.apiKey);
    const headers = {
    ...
    try {
        ...
        if (timeoutController?.signal.aborted && !(error instanceof LLMTimeoutError) && timeoutMs) {
            throw new LLMTimeoutError(`LLM request timed out after ${timeoutMs}ms`, timeoutMs);
        }
        throw error;
    }
    finally {
...
// ---------------------------------------------------------------------------
async function executeChatCompletion(config, model, messages, temperature, maxTokens, options) {
    const serviceName = config.service ?? "custom";
    const serviceConfig = getServiceConfig(serviceName);
    const baseUrl = config.baseUrl || serviceConfig?.baseUrl || "";
    ...
    const authToken = providerRegistry.resolveAuthToken(serviceName, config.oauthToken ?? config.apiKey);
