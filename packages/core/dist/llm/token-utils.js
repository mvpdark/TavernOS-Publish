 ...
 */
export function checkContextWindow(inputTokens, serviceName, modelName, reservedOutput) {
 ...
 */
export function assertWithinContextWindow(inputTokens, serviceName, modelName, reservedOutput) {
    const check = checkContextWindow(inputTokens, serviceName, modelName, reservedOutput);
    if (!check.withinWindow) {
        throw new ContextWindowExceededError(`Estimated input tokens (${check.inputTokens}) exceed available context window ` +
            `(${check.available}/${check.contextWindow} after reserving ${check.reservedOutput} for output). ` +
            `Reduce the input or use a model with a larger context window.`, check.inputTokens, check.contextWindow, check.reservedOutput);
    }
}
// ---------------------------------------------------------------------------
 ...
 */
export function clampTemperatureForModel(serviceName, modelName, temperature) {
    const modelCard = lookupModel(serviceName, modelName);
