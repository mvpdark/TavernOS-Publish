...
export class ProviderRegistry {
     ...
     */
    isModelValid(serviceId, modelName) {
        const service = this.table.get(serviceId);
        if (!service)
            return true; // Custom services allow any model
        if (service.models.length === 0)
            return true; // Services with empty model lists allow any
     ...
     */
    resolveAuthToken(serviceId, apiKey) {
        const service = this.table.get(serviceId);
        if (!service)
            return apiKey; // Custom services use apiKey directly
...
// ---------------------------------------------------------------------------
export const providerRegistry = new ProviderRegistry(PROVIDER_CONFIGS);
// ---------------------------------------------------------------------------
...
export function lookupModel(serviceName, modelName) {
    return providerRegistry.findModel(serviceName, modelName);
}
export function validateModelBelongsToService(serviceName, modelName) {
...
//# sourceMappingURL=provider-bank.js.map