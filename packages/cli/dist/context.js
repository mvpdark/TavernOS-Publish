...
export async function resolveCliContext() {
    ...
    if (!projectRoot) {
        throw new Error("Not inside a TavernOS project. Run `tavernos init <name>` to create one.");
    }
 ...
 */
export async function resolveBookId(ctx, explicit) {
    if (explicit)
        return explicit;
    ...
    if (ids.length === 1)
        return ids[0];
    throw new Error(`Multiple books found (${ids.join(", ")}). Specify --book <id>.`);
}
