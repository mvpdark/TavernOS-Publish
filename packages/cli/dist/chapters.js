 ...
 */
export async function loadChapter(projectRoot, bookId, number) {
    const filePath = join(chaptersDir(projectRoot, bookId), chapterFileName(number));
    return readJson(filePath);
}
/** Save a chapter (including body) to disk. */
export async function saveChapter(projectRoot, bookId, chapter) {
 ...
 */
export async function listChapters(projectRoot, bookId) {
    ...
    try {
        ...
        for (const entry of entries) {
            if (!entry.endsWith(".json"))
                continue;
            const full = await readJson(join(dir, entry));
            if (full) {
...
export async function nextChapterNumber(projectRoot, bookId) {
